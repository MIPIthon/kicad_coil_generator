...
# Used to store all traces on F.Cu
segments_fcu = []

# Used to store all traces on B.Cu 
segments_bcu = []

# Used to store all traces on both copper layers 
segments_all = []

# Define the name of the footprint
fp_name = "example_coil_rev10_5"
...
# **********************************************************************************
# FIX #4: Add comprehensive input validation
def validate_coil_parameters(p):
    """
    Validates coil parameter dictionary for physical and logical constraints.
    Raises ValueError if any parameter is invalid.
    """
    errors = []
    
    # Validate rr (radius ratio) is in range [0, 1]
    if not isinstance(p.get('rr'), (int, float)) or not (0 <= p['rr'] <= 1):
        errors.append(f"rr must be in range [0, 1], got {p.get('rr')}")
    
    # Validate spacing is positive
    if not isinstance(p.get('spacing'), (int, float)) or p['spacing'] <= 0:
        errors.append(f"spacing must be > 0, got {p.get('spacing')}")
    
    # Validate n is a positive integer
    if not isinstance(p.get('n'), int) or p['n'] <= 0:
        errors.append(f"n must be a positive integer, got {p.get('n')}")
    
    # Validate width is positive
    if not isinstance(p.get('width'), (int, float)) or p['width'] <= 0:
        errors.append(f"width must be > 0, got {p.get('width')}")
    
    # Validate gap is positive
    if not isinstance(p.get('gap'), (int, float)) or p['gap'] <= 0:
        errors.append(f"gap must be > 0, got {p.get('gap')}")
    
    # Validate inner dimensions are positive
    if not isinstance(p.get('w_in'), (int, float)) or p['w_in'] <= 0:
        errors.append(f"w_in must be > 0, got {p.get('w_in')}")
    
    if not isinstance(p.get('h_in'), (int, float)) or p['h_in'] <= 0:
        errors.append(f"h_in must be > 0, got {p.get('h_in')}")
    
    # Validate connection type
    if p.get('conn') not in ["in", "out", ""]:
        errors.append(f"conn must be 'in', 'out', or empty string, got {p.get('conn')}")
...
param_ext_conn_1 = {
    ...
    'size_x' : 1.8,             # in mm (used, outer pad size in x dir.) 
...
param_2L_coil = {
    ...
    'w_in': 5,                  # in mm
    'h_in': 5,                  # in mm
    'n': 10,                    # number of windings per layer (no unit)
    'gap': 2,                   # in mm
    ...
    'conn_pad_x_spacing': 5,    # Spacing between both pads in x direction
...
def fct_fp_line_len_res(xs, ys, xe, ye, width, layer):
    ...
    if layer == "F.Cu":
        # FIX #2 & #3: Consistent segment ordering and populate segments_all
        segment_fcu = np.array([[xs*1E-3, ys*1E-3, 0], [xe*1E-3, ye*1E-3, 0]], dtype=float)
        segments_fcu.append(segment_fcu)
        segments_all.append(segment_fcu)
    if layer == "B.Cu":
        # FIX #2 & #3: Fixed segment ordering from [[xe, ye], [xs, ys]] to [[xs, ys], [xe, ys]] for consistency
        # Also populate segments_all
        segment_bcu = np.array([[xs*1E-3, ys*1E-3, pcb_thickness*1E-3], [xe*1E-3, ye*1E-3, pcb_thickness*1E-3]], dtype=float)
        segments_bcu.append(segment_bcu)
        segments_all.append(segment_bcu)
...
if __name__ == "__main__":
    # FIX #4: Validate parameters before execution
    try:
        validate_coil_parameters(param_2L_coil)
    ...
    # FIX #5: Improved file error handling
    files_created = []
    try:
        with open(file_name_2D_plot, "x") as f:
            ...
            files_created.append(file_name_2D_plot)
    
    except FileExistsError:
        print('File "'+file_name_2D_plot+'" already exists! Skipping 2D plot save.')
    ...
    try:
        with open(file_name_3D_plot, "x") as f:
            ...
            files_created.append(file_name_3D_plot)
    
    except FileExistsError:
        print('File "'+file_name_3D_plot+'" already exists! Skipping 3D plot save.')
    ...
    try:
        with open(fp_name+file_ending_footprint, "x", encoding="utf-8") as f:
            ...
            f.writelines(out)
            files_created.append(fp_name+file_ending_footprint)
    except FileExistsError:
        print('File "'+fp_name+file_ending_footprint+'" already exists! Skipping footprint save.')
    ...
    try:
        with open(fp_name+file_ending_config, "x") as f:
            ...
            files_created.append(fp_name+file_ending_config)
    except FileExistsError:
        print('File "'+fp_name+file_ending_config+'" already exists! Skipping config save.')
    
    print(f"\n✓ Successfully created {len(files_created)} file(s): {', '.join(files_created)}")
